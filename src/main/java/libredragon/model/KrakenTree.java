...
public class KrakenTree {
	...
	private HashMap<String,ArrayList<Pair<Expression, Pair<Rule,String>>>> rules = new HashMap<>();
	...
	public boolean ruleIsIn(String exprid, int idrule, String context){
		if( rules.containsKey(exprid) && rules.get(exprid).size() > idrule && rules.get(exprid).get(idrule).second.second.compareTo(context) == 0)
					return true;
		return false;
	}
	...
	public ArrayList<Pair<Expression, Pair<Rule,String>>> getRules(String id){
		return rules.get(id);
	}

	public void addRules(String id, ArrayList<Pair<Expression, Pair<Rule,String>>> liste){
		rules.put(id, liste);
	}

	public Expression getVarianteByIds(String exprid, int idrule, String context){
	...
	public Rule getRuleByidsAndContext(String exprid, int idrule, String context){
		if(idIsIn(exprid) && ruleIsIn(exprid, idrule, context))
			return getRules(exprid).get(idrule).second.first;
		return null;
	}
	 ...
	 * @return
	 */
	public ArrayList<Pair<Expression,Pair<Rule,String>>> generateRules(Expression expression){
		ArrayList<Pair<Expression,Pair<Rule,String>>> res = new ArrayList<>();
		generateRulesForExpression(expression, res);
		ExpressionApi a = new ExpressionApi();
		ArrayList<Expression> liste = a.run(expression);
		removeDoublon(liste, expression);
		for(int i = 0; i < liste.size(); i ++){
			generateRulesForExpression(liste.get(i), res);
		}
		return res;
	}

	private void generateRulesForExpression(Expression expression, ArrayList<Pair<Expression,Pair<Rule,String>>> expRules){
		List<Rule> liste;
		...
		while(iterateur.hasNext())
		{
			...
			for(int i = 0; i < liste.size(); i++){
				if (liste.get(i).canApplic(expression)){
					expRules.add(new Pair (expression, new Pair(liste.get(i), key)));
				}
			}
		}
	}
	...
	public void replaceExpression (Expression oldexp, Expression newExp){
		Expression father = oldexp.getFather();
		if(father instanceof BinaryExpression){
			BinaryExpression bfather = (BinaryExpression) father;
			if(oldexp.compare(bfather.firstExpression()))
				bfather.setFirstExpression(newExp);
			else if(oldexp.compare(bfather.secondExpression()))
				bfather.setSecondExpression(newExp);
		}
		else if(father instanceof UnaryExpression){
			UnaryExpression ufather = (UnaryExpression) father;
			ufather.setSubExpression(newExp);
		}
	}

	private  void removeDoublon(ArrayList<Expression> liste, Expression expression){
		for (int i = 0; i < liste.size(); i++){
			for (int j = i + 1; j < liste.size(); j++)
				if(liste.get(i).compare(liste.get(j))){
					liste.remove(j);
					j--;
				}
				if(expression.compare(liste.get(i))){
					liste.remove(i);
					i--;
				}
			}
