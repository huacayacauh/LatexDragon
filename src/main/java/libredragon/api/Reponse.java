...
public class Reponse {
	...
	private String generateJsonRules(KrakenTree tree){
		...
		while(iterateur.hasNext())
		{
			...
			if(liste.size() > 0){
				temp += ("{\"text\":\"$$"+exp.generateExpression("") + " => "+liste.get(0).second.first.applic(liste.get(0).first).generateExpression("")+"$$\","+"\"ruleId\":"+0+",\"type\":"+"\""+liste.get(0).second.second+"\"}");
			}
			for(int i = 1; i < liste.size(); i++){
				temp += ","+("{\"text\":\"$$"+exp.generateExpression("") + " => "+liste.get(i).second.first.applic(liste.get(i).first).generateExpression("")+"$$\","+"\"ruleId\":"+i+",\"type\":"+"\""+liste.get(i).second.second+"\"}");
			}
