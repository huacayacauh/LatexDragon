...
public class Reponse {
	...
	private String generateJsonRules(KrakenTree tree){
		...
		while(iterateur.hasNext())
		{
			...
			temp += "{\""+key +"\":[";
			exp = tree.getIds(key);
			ArrayList<Pair<Expression,Pair<Rule,String>>> liste = tree.getRules(key);
			if(liste.size() > 0){
				temp += ("{\"text\":\"$$"+exp.generateSimpleExpression() + " => "+liste.get(0).second.first.applic(liste.get(0).first).generateSimpleExpression()+"$$\","+"\"ruleId\":"+0+",\"type\":"+"\""+liste.get(0).second.second+"\"}");
			}
			for(int i = 1; i < liste.size(); i++){
				temp += ","+("{\"text\":\"$$"+exp.generateSimpleExpression() + " => "+liste.get(i).second.first.applic(liste.get(i).first).generateSimpleExpression()+"$$\","+"\"ruleId\":"+i+",\"type\":"+"\""+liste.get(i).second.second+"\"}");
			}
