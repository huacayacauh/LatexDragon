...
var self = module.exports = {
  ...
  setEvents: () => {
		...
		var obj = instance.gameState.currentState
    ...
    for (var i in obj.ids) {
        ...
        $("#"+obj.ids[i]).on("contextmenu", { value:obj }, self.contextmenuHandler);
				...
				if (instance.settings.highlighting) {
					$("#"+obj.ids[i]).on("mouseover", function () { $(this).css('text-shadow', '0 0 8px ' + instance.settings.highlightColor) })
					$("#"+obj.ids[i]).on("mouseleave", function () { $(this).css('text-shadow', '') })
					$("#"+obj.ids[i]).on("mouseout", function () { $(this).css('text-shadow', '') })
				}
    }
  },
